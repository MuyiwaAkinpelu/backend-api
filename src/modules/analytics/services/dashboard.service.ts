...
export class DashboardService {
  ...
  async getStats(user: User) {
    ...
    try {
      ...
      this.logger.error(
        `Error getting dashboard stats for user ${user.id}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  private async getSystemAdminStats() {
    const [docStats, approvalStats, projects] = await Promise.all([
    ...
    const [
      ...
      this.docAnalytics.getMetricWithTrend(
        { approvalRequests: { some: { status: ApprovalStatus.APPROVED } } },
        true,
      ),
    ...
    return {
      ...
      data: {
        ...
        recentUploads: recentGlobalApprovedUploads.map((u) => ({
          ...u,
  ...
  private async getManagementStaffStats(user: User) {
    const projectIds = [
      ...(user.projectManagerProjectIDs || []),
      ...(user.projectMemberProjectIDs || []),
    ];

    const [docStats, approvalStats, projects] = await Promise.all([
      this.docAnalytics.getSummaryStats({
  ...
  private async getOperationalStaffStats(user: User) {
    ...
    const [docStats, submissionStats] = await Promise.all([
      this.docAnalytics.getSummaryStats({
        uploaderId: user.id,
        approvalRequests: { some: {} },
      }),
    ...
    const dashboardProjectIds = projects.map((p) => p.id);
    const projectsStats = await this.projectAnalytics.getProjectsStatsBatch(
      dashboardProjectIds,
    );
  ...
  private async getNewStaffStats(user: User) {
    // "Basic Staff"
    const docStats = await this.docAnalytics.getSummaryStats({
      approvalRequests: { some: {} },
    });
