...
export class NotificationService {
  ...
  async createNotification(payload: ICreateSingleNotificationPayload) {
    const canReceive = await this.notificationPreferenceService.canReceive(
      payload.userId,
      payload.type,
      'push',
    );
  ...
  async markOneNotificationAsRead(
    userId: string,
  ...
  async getAllMemberNotifications(userId: string, page = 1, limit = 15) {
    try {
      ...
      this.logger.error(
        `Failed to get all notifications for user ${userId}`,
        error,
      );
      return { data: [], meta: {} };
    }
  ...
  async handleApprovalStatusChanged(payload: {
    userId: string;
    status: string;
    projectName: string;
    requestId: string;
