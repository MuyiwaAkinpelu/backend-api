...
export class NotificationPreferenceService {
    ...
    async canReceive(userId: string, type: NotificationType, channel: 'push' | 'email' = 'push'): Promise<boolean> {
        ...
        if (type === NotificationType.PROJECT_UPDATE || type === NotificationType.DOCUMENT_UPDATED) {
            if (channel === 'email') return preference.email && preference.updates;
            return preference.push && preference.updates;
        }

        if (type === NotificationType.APPROVAL_STATUS || type === NotificationType.DOCUMENT_APPROVED || type === NotificationType.DOCUMENT_DECLINED || type === NotificationType.REQUEST_SUBMITTED) {
